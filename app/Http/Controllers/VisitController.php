...
class VisitController extends Controller
{
    ...
    public function orderReport(Request $request)
    {
        ...
        $responsible_order_type[0] = 'done';
        $responsible_order_type[1] = 'missed';

        $data['responsible_order_type'] = $responsible_order_type;
        ...
        $start_date = ($request->input("start_date")) ? $request->input("start_date") : '';
        $end_date = ($request->input("end_date")) ? $request->input("end_date") : '';
        $responsible_order_selected = $request->input("responsible_order_selected");
        ...
        $selected_fo_ids = $request->input('selected_fo_ids');
        $data['selected_fo_ids'] = $selected_fo_ids;
        $data['json_fo_ids'] = json_encode($selected_fo_ids);

        $data['users'] = $users;
        $data['start_date'] = $start_date;
        $data['end_date'] = $end_date;
        $data['responsible_order_selected'] = $responsible_order_selected;

        return view('visits.form_order_report', $data);
    }
    ...
    public function getOrderVisits(Request $request)
    {

        $start_date = $request->input('start_date');
        $end_date = $request->input('end_date');
        if (request()->session()->get('connected_user_acces') != 'Responsible')
            $responsible_order_selected = $request->input("responsible_order_selected");
        else $responsible_order_selected = -1;
        $zone_ids = json_decode($request->input('zone_ids'));
        $fo_ids = json_decode($request->input('fo_ids'));
        ...
        $visits = $this->visitRepository->getAjaxOrderVisits($start, $limit, $order, $dir, $draw, $search,
            $start_date, $end_date, $zone_ids, $fo_ids,$responsible_order_selected);

        //dd($visits);
        echo json_encode($visits);
    }

    public function postOrderFile(Request $request)
    {
        $this->validate($request, [
            'file' => 'required',
        ]);
        $save['visit_id'] = $request->input('visit_id');
        ...
        if ($request->file('file')) {
            ...
            $file->move(public_path('OrderResponsible'), $randomFilename.'.'.$extension);
            $save['file_name'] = $filename;
            $save['random_file_name'] = $randomFilename.'.'.$extension;
        }

        $id_file_inserted = $this->visitRepository->addFile($save);
    ...
    public function report($visit_id)
    {
       ...
       {
           return redirect('visit')->withErrors('Visit Not Found');
       }
